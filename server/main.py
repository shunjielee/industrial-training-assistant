...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Indexing documents from: {settings.PDF_FOLDER}")
    index_result = indexer.index_directory(settings.PDF_FOLDER)
...
def reindex_documents():
    ...
    try:
        result = indexer.index_directory(settings.PDF_FOLDER)
...
def login(req: LoginRequest):
    ...
    if user_id in users and users[user_id]["password"] == req.password:
        return LoginResponse(
            success=True,
            message="Login successful",
            user_id=user_id,
            user_type=users[user_id].get("user_type", "student")
        )
...
def register(req: RegisterRequest):
    ...
    users[user_id] = {
        "password": req.password,
        "user_type": req.user_type if req.user_type in ["student", "teacher"] else "student"
    }
    save_users(users)
    ...
    return LoginResponse(
        ...
        user_id=user_id,
        user_type=users[user_id]["user_type"]
    )
